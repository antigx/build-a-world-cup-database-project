...
echo "$($PSQL "select name from teams inner join games on teams.team_id=games.winner_id where year = 2014 and round = 'Eighth-Final'
                union 
                select name from teams inner join games on teams.team_id=games.opponent_id where year = 2014 and round = 'Eighth-Final' order by name")"
